...
// ── In-memory maintenance flag (persists until server restart) ──
// On Render free tier, server may restart; for persistence use DB toggle below.
...
// ── Maintenance Mode: public status endpoint ─────────────────
// Frontend calls this on every page load to decide whether to show maintenance page
app.get("/api/public/maintenance", (_req, res) => {
  res.json({ maintenance: maintenanceMode });
});

// ── Admin: Get + Toggle Maintenance Mode ─────────────────────
app.get("/api/admin/maintenance", requireAdmin, (_req, res) => {
  res.json({ maintenance: maintenanceMode });
});

app.post("/api/admin/maintenance", requireAdmin, asyncHandler(async (req, res) => {
  const schema = z.object({ maintenance: z.boolean() });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid input" });

  maintenanceMode = parsed.data.maintenance;
...
app.post("/api/public/internships/apply", asyncHandler(async (req, res) => {
  ...
  const { studentName, studentEmail, studentPhone, studentCollege, studentDegree, internshipTrack, studentSkills, studentStatement } = parsed.data;
  await pool.query(
...
app.post("/api/admin/clients", requireAdmin, asyncHandler(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO clients (name, industry, summary) VALUES ($1, $2, $3) RETURNING id, name, industry, summary`,
...
app.get("/api/admin/careers", requireAdmin, asyncHandler(async (_req, res) => {
  const { rows } = await pool.query(
    `SELECT id, title, location, employment_type, experience_level, description, is_active FROM career_openings ORDER BY id DESC`
  );
  res.json(rows);
}));
...
app.post("/api/admin/careers", requireAdmin, asyncHandler(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO career_openings (title, location, employment_type, experience_level, description) VALUES ($1,$2,$3,$4,$5) RETURNING *`,
...
app.get("/api/admin/internships", requireAdmin, asyncHandler(async (_req, res) => {
  const { rows } = await pool.query(
    `SELECT id, title, duration, mode, description, is_active FROM internships ORDER BY id DESC`
  );
  res.json(rows);
}));
...
app.post("/api/admin/internships", requireAdmin, asyncHandler(async (req, res) => {
  ...
  const { rows } = await pool.query(
    `INSERT INTO internships (title, duration, mode, description) VALUES ($1,$2,$3,$4) RETURNING *`,
...
app.patch("/api/admin/applications/:id/status", requireAdmin, asyncHandler(async (req, res) => {
  ...
  const schema = z.object({ status: z.enum(["new", "reviewing", "accepted", "rejected"]) });
  const parsed = schema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "Invalid status" });

  await pool.query("UPDATE internship_applications SET status = $1 WHERE id = $2", [parsed.data.status, id]);
  res.json({ ok: true });
}));
